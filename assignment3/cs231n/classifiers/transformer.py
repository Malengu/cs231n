...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        tgt_pos = self.positional_encoding(tgt) # (N, T, W)
        img_features = self.visual_projection(features)[:, None, :] # (N, 1, W)
        tgt_mask = torch.tril(torch.ones(T, T)) # (T, T)
        decoder_features = self.transformer(tgt_pos, img_features, tgt_mask) # (N, T, W)
        scores = self.output(decoder_features) # (N, T, W) @ (W, V) -> (N, T, V)
