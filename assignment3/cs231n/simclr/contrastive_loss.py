...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k = -torch.log(num / denom_k)
        l_k_N = -torch.log(num / denom_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(out_left_norm*out_right_norm, dim=1, keepdims=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = sim_positive_pairs(out[:N, :], out[N:, :])
    ...
    numerator = torch.cat([torch.exp(pos_pairs / tau) for _ in range(2)], dim=0)
